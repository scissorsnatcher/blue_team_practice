	...
	public void fillEdge(int i, int j, int k, Color c, boolean flag) {
		...
		//	g.drawLine(nodes.get(e.getSrc()).getX(), nodes.get(e.getSrc()).getY(),nodes.get(e.getDest()).getX(),  nodes.get(e.getDest()).getY());
			...
			AffineTransform old = g2d.getTransform();
					/*int to_x,to_y;
					if(nodes.get(e.getDest()).getX() <= nodes.get(e.getSrc()).getX()){
						to_x = nodes.get(e.getDest()).getX();
						to_y = nodes.get(e.getDest()).getY();
					}
					else {
						to_x = nodes.get(e.getSrc()).getX();
						to_y = nodes.get(e.getDest()).getY();
					}
*/
			g2d.rotate(Math.toRadians(360 - angle_3),circlex  ,circley);





			g.drawArc(circlex-radius, circley -radius/2 ,radius*2,  radius, 0, 180);


			//System.out.println(angle_3 );
			g.drawString(e.getWeightStr(), circlex, circley -radius/2 );
			...
			g.drawString( e.getWeightStr(), circlex, circley +radius/2 );
			g.drawArc(circlex-radius, circley -radius/2 ,radius*2,  radius, 180, 360);
...
*/
					...
					if(e.m_flag) {
						...
						if(nodes.get(e.getDest()).getY() > nodes.get(e.getSrc()).getY())
							to_y =nodes.get(e.getDest()).getY() +2*radius ;
						else
							to_y = nodes.get(e.getSrc()).getY() +2* radius;
*/

					/*	Graphics2D g2d = (Graphics2D)g;
						AffineTransform old = g2d.getTransform();
						g2d.rotate(Math.toRadians(angle_3));
						//AffineTransform a = new AffineTransform();
						g.drawOval(to_x,to_y,2*radius,radius );
						g2d.setTransform(old);*/
						//a.rotate(67);
